...
If $\alpha$ is not regular, then we call it \define{Singular}.
\end{definition}
...
% HW 3:
% 1.4 #2 (Find the unique curve $\alpha(0)=(1,0,5)$ and $\alpha'(t)=(t^{2},t,\E^{t})$.
% 2.2 #3,4,5
% 2.3 #1,5,6


%% At this point, homework 3 would be due.
...
% \item Given a unit-speed parametrization of the unit circle on the (x,y)-plane in $\RR^{3}$, rotated by some
% random Euler-angles, then translated by a random vector (5,4,-2),
  % give this transformed curve $\beta(s)$.
  % Ask the reader to verify $\beta(s)$ is a unit-speed curve and then compute the Frenet data for $\beta$.
...
\define{Osculating Circle}, and $\vec{c}$ is called the \define{Center of Curvature} of $\beta$ at $\beta(0)$.
...
\N{Frame field as partial derivatives}
...
\M We begin with $\omega_{12}[V] = \langle\nabla_{V}E_{1},E_{2}\rangle$.
...
+v^{3}\cos(\lambda)U_{3}.
  \end{split}
\end{equation}
Now we take the dot product with $E_{2}$ (recall
...
\N{Using the Covariant Derivative}
If we want to know how fast the vector field $W$ is changing with
...
%% At this point, homework 4 would be due.
%% It consisted of the following
%% exercises from O'Neill's \emph{Elementary Differential Geometry}
%% (Revised second ed.),
%% \begin{itemize}
%% \item 2.4 \# 1,3
%% \item 2.5 \# 2(a, b, f); 3; 5a.
%% \end{itemize}
...
\item Prove or find a counter-example: if $\Sigma\subset\RR^{n}$ is a
  ...
  on $\alpha$ called the \define{Restriction} of $W$ to $\alpha$ and
