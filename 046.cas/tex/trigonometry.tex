...
\begin{calculation}
\sin^{2}(x/2)
\step{since $\sin^{2}(x/2) + \cos^{2}(x/2) = 1$}
1 - \cos^{2}(x/2)
\step{using Proposition~\ref{prop:trigonometry:half-angle-formula-for-cosine}}
1 - \frac{1 + \cos(x)}{2}
\step{algebra}
\frac{1 - \cos(x)}{2}.\qedhere
\end{calculation}
\end{proof}
...
We have seen some exact values already, Axiom~\ref{axiom:trigonometry:sine-pi-over-2-equals-1}
...
\step{using Proposition~\ref{prop:trigonometry:half-angle-formula-for-cosine}}
  \frac{1 + \cos(\pi/2^{2+n})}{2}
\step{multiply top and bottom by $2$}
  \frac{2 + 2\cos(\pi/2^{2 + n})}{4}.
\end{calculation}
...
\begin{calculation}
\cos(x/2)
\step[\leq]{since $-1\leq\cos(x/2)\leq 1$}
\cos^{2}(x/2)
\step{half-angle formula from Proposition~\ref{prop:trigonometry:half-angle-formula-for-cosine}}
\frac{1 + \cos(x)}{2}
\step[\leq]{by hypothesis}
\frac{1 + 1 - k}{2},
\end{calculation}
hence the result.
\end{proof}
...
\begin{calculation}
  \cos(x)
\step{using cosine double-angle formula}
2\cos^{2}(x/2) - 1
\step[\leq]{since $\cos^{2}(x/2)\leq 1 - k/2$}
2(1 - k/2) - 1 = 1 - k.
\end{calculation}
Hence the result.
...
\begin{calculation}
\sin(x/2)
\step[\leq]{since $-1\leq\sin(x/2)\leq 1$}
\sin^{2}(x/2)
\step{using Proposition~\ref{prop:trigonometry:half-angle-formula-for-sine}}
\frac{1 - \cos(x)}{2}
\step[<]{by hypothesis}
\frac{1 - (1 + k)}{2}
\step{algebra}
...
\begin{calculation}
\cos(x)
\step{since $\cos^{2}(x) = 1-\sin^{2}(x)$}
\sqrt{1 - \sin^{2}(x)}
\step[\leq]{upper bound of squareroot}
1 - \frac{1}{2}\sin^{2}(x)
\step[\leq]{by hypothesis}
1 - \frac{1}{2}k^{2}.\qedhere
\end{calculation}
\end{proof}
...
\step{half-angle law}
\frac{1 - \cos(x)}{2}
\step[\leq]{using previous result}
\frac{1 - (1 - k^{2}/2)}{2} = \frac{k^{2}}{4}.
\end{calculation}
...
\begin{calculation}
\sin\bigl((k+1)x\bigr)
\step{angle-addition law}
\sin(kx)\cos(x) + \cos(kx)\sin(x)
\step[\leq]{since $\cos(x)\leq1$}
\sin(kx) + \sin(x)
\step[<]{base case}
\sin(kx) + x
