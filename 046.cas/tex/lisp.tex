...
\N*{Step 0} If \lstinline[language=lisp]{dict} is
...
\N*{Step 3} When we're trying to match a pattern segment, well, it's a
...
\N*{Step 5} We have the head of the \lstinline[language=lisp]{input} be
...
\N*{Step 2: depth-first search for longest fit}
...
\N*{Step 0} If the skeleton is \lstinline[language=lisp]{nil},
then we return \lstinline[language=lisp]{nil}.

\N*{Step 1} If the skeleton is a pattern variable
\lstinline[language=lisp]{(? x)}, then we lookup the value in our
dictionary and retun that.

\N*{Step 2}
If the skeleton is a list, then we have several subcases:
\begin{enumerate}
