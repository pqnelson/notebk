...
\M\index{Lagrangian!Scalar field}
...
\begin{definition}\index{Klein--Gordon!Field}\index{Field!Scalar|see{Scalar Field}}\index{Scalar Field!Free}
...
We define the \define*{Hamiltonian Vector Field}\index{Hamiltonian!Vector field} as the vector field $X$
on $\Gamma$ defined by
\begin{equation}
X = \Omega^{-1}(-,\D H).
\end{equation}
...
Now, if we shift to the situation in field theory, we have $A$ and $B$
